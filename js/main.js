...
$(document).ready(function () {
  ...
  function resetGame() {
    ...
    TIME_LEFT = 99;
    SCORE = 0;
    ASSIST_TIME_CREDITS = 3;
    ASSIST_CLUE_CREDITS = 3;
    CUR_IMG_IND = 0;

    // Reset the image pool (fresh copies of the original answers)
    // We need to deep-copy the original objects to avoid mutation.
    IMAGES = [
      JSON.parse(JSON.stringify(img1)),
      JSON.parse(JSON.stringify(img2)),
      JSON.parse(JSON.stringify(img3)),
      JSON.parse(JSON.stringify(img4)),
      JSON.parse(JSON.stringify(img5))
    ];
    // Ensure each copied object has the right 'cssClass' (string) and empty 'found' array
    IMAGES.forEach(function(img, idx) {
      img.cssClass = 'img' + (idx+1);
      img.found = [];
    });
    ...
    $('#assist-time img').show();
    $('#assist-clue img').show();

    // Clear both canvases
    clearCanvas();

    // Pick a new random image and serve it
    CUR_IMG_IND = randomIntFromInterval(0, IMAGES.length - 1);
    CUR_IMG_IN_PLAY = IMAGES[CUR_IMG_IND];
    serveNewImg(CUR_IMG_IN_PLAY);
